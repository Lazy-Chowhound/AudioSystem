...
class LibriSpeech(Dataset):
    ...
    def get_name_and_pattern_tag(self, name):
        ...
        """
        name = name.replace("\\", "/")
        return name[:name.find("_")] + ".flac", name[name.find("_") + 1:name.rfind(".")]
    ...
    def get_noise_testset_audio_clips_list(self):
        ...
        :return:
        """
        noise_testset_path = self.noise_clips_path + "test-clean/"
        audio_list = glob.glob(noise_testset_path + "*/*/*.flac")
        audios = []
        for audio in audio_list:
            if get_audio_form(audio) == "flac":
